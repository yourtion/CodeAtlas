...
> 注：已移除 AGE 图数据库支持，改用关系表（edges/symbols）的 SQL 查询实现关系遍历。
...
func (h *RelationshipHandler) GetCallers(c *gin.Context) {
    ...
    rows, _ := h.db.QueryContext(ctx, query, symbolID)
    // 处理结果...
}
```
...
func (h *RelationshipHandler) GetCallees(c *gin.Context) {
    ...
    query := `
        ...
        WHERE e.source_id = $1 AND e.edge_type = 'call'
    `
}
```
...
func (h *RelationshipHandler) GetDependencies(c *gin.Context) {
    query := `
        SELECT s.id, s.name, e.edge_type
        FROM edges e
        JOIN symbols s ON s.id = e.target_id
        WHERE e.source_id = $1
          AND e.edge_type IN ('import', 'extends', 'implements', 'references')
    `
}
```
...
A: 关系查询基于关系表（edges）的标准 SQL，请检查 edges 表数据与索引是否正确:

```go
rows, err := h.db.QueryContext(ctx, query, symbolID)
if err != nil {
    c.JSON(http.StatusInternalServerError, gin.H{
