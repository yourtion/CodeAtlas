...
func TestSymbolRepository_GetByFileID(t *testing.T) {
	...
	retrieved, err := repo.GetByFileID(ctx, fileID)
	if err != nil {
...
func TestSymbolRepository_GetByKind(t *testing.T) {
	...
	functions, err := repo.GetByKind(ctx, fileID, "function")
	if err != nil {
	...
	// Get class symbols
	classes, err := repo.GetByKind(ctx, fileID, "class")
	if err != nil {
...
func TestSymbolRepository_GetByName(t *testing.T) {
	...
	if len(testSymbols) != 2 {
		t.Errorf("Expected 2 symbols with 'Test' in name, got %d", len(testSymbols))
	}
...
func TestSymbolRepository_GetSymbolsWithDocstrings(t *testing.T) {
	...
	documented, err := repo.GetSymbolsWithDocstrings(ctx, fileID)
	if err != nil {
...
func TestSymbolRepository_CountByKind(t *testing.T) {
	...
	// Get counts by kind
	counts, err := repo.CountByKind(ctx, fileID)
	if err != nil {
